...
(function () {
  ...
  rpcMethods.runMain = async (params) => {
    ...
    try {
      // DDC invokes this hook whenever main throws (sync or async). Surface
      // it so the embedder can distinguish a crash from a clean exit.
      self.dartDevEmbedder.config.mainErrorCallback = () => {
        sendNotification('error', { message: 'Unhandled exception in main' });
      };
